...
*Your answer should have the following format:* `XXXXXXXX, XXXXXXXX, XXXXXXXX` *where the* `X`*s are replaced by* `0` *or* `1`*. If a certain number can't be represented, leave the* `X`*s.*
...
*Your answer should have the following format:* `XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX` *where the* `X`*s are replaced by* `0` *or* `1` *(spaces must be added between packs of 4 bits)*
...
Il sera donc forcément impossible de représenter précisément 0.1 en binaire. On va donc tronquer la partie décimale selon un *scaling factor* (qui est le nombre de bits que l'on décide de garder pour la partie décimale. Par exemple, si $k = 5$, on dira que $0.1_{10} \simeq 0.00010_2 \simeq 0.00011_2$ en fonction de comment on arrondit le nombre).
...
::: info Note
Il n'est pas important de comprendre la notation du *float64* par cœur pour comprendre cet exemple. Concentrez-vous uniquement sur la partie décimale.
:::
